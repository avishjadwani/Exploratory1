...
#plot1 Histogram freq vs global acitve power
hist(mydata$Global_active_power)
# Error could not function this
#Solution (Google & Stack overflow help) : Global active power is a factor variable
#class(mydata$Global_active_power)
#We have to convert it to numeric data
#I tried converting directly to numeric but graph looks different 
#Its given that to efficiently convert a factor, convert to character and the to numeric
# *** If you can explain better the please do in comment section
hist(as.numeric(as.character(filtereddata$Global_active_power)),main="Global Active Power",xlab = "Global Active Power (Killowatts)",col = "red")
#Yeah ! One  Done 
